...
import numpy as np
import pytest
from pyiks.anomaly import IDKD
...
def test_inne_performance():
    ...
    # fit the model
    clf = IDKD(n_estimators=100, max_samples=16).fit(X_train)
...
def test_fit_time():
    data = digit.data
    print(data.shape)
    clf = IDKD(n_estimators=200, max_samples=100)
    t1 = time.time()
    clf.fit(data)
    t2 = time.time()
