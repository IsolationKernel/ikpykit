...
| --------------- | ------- | -------------------------------------------------------------- |
| `n_features`    | `int`   | Number of features in the dataset (2: longitude and latitude). |
| `n_samples`     | `int`   | Total number of trajectory samples after processing.           |
| `n_classes`     | `int`   | Number of classes (2: normal and anomalous).                   |
| `anomaly_ratio` | `float` | Ratio of anomalous trajectories to total trajectories.         |
...
| Name   | Description                 |
| ------ | --------------------------- |
| `load` | Load the SheepDogs dataset. |
...
| ------------ | ------ | ------------------------------------------------------------------------------------------------------------------------ | ------- |
| `return_X_y` | `bool` | If True, returns a tuple (X, y) where X is the data and y is the target. If False, returns a dict with keys 'X' and 'y'. | `False` |

Returns:

| Type            | Description                                                                                                                                              |
| --------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `dict or tuple` | Either (X, y) tuple or {'X': data, 'y': target} dict where data is a list of trajectories and target indicates normal (1) or anomalous (0) trajectories. |
